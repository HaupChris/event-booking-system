...
interface FormContent {
  ticket_options: { id: number; title: string }[];
  beverage_options: { id: number; title: string }[];
  food_options: { id: number; title: string }[];
  work_shifts: { id: number; title: string; time_slots: { id: number; title: string, start_time: string, end_time: string }[] }[];
...
const BookingsPage: React.FC = () => {
  ...
  const getTimeslotWithWorkshift = (id: number | string | number[]) => {
    for (const shift of formContent.work_shifts) {
      ...
      if (timeslot) {
        return { workshiftTitle: shift.title, timeslotTitle: timeslot.title, timeslotStart: timeslot.start_time, timeslotEnd: timeslot.end_time };
      }
    }
        ...
        <Box sx={{
          ...
          {selectedBooking && (
            <Box>
              ...
              <Typography variant="body1"><strong>First Name:</strong> {selectedBooking.first_name}</Typography>
              <Typography variant="body1"><strong>Last Name:</strong> {selectedBooking.last_name}</Typography>
              ...
              <Typography variant="body1"><strong>Ticket:</strong> {getOptionTitle(selectedBooking.ticket_id, formContent.ticket_options)}</Typography>
              <Typography variant="body1"><strong>Beverage:</strong> {getOptionTitle(selectedBooking.beverage_id, formContent.beverage_options)}</Typography>
              <Typography variant="body1"><strong>Food:</strong> {getOptionTitle(selectedBooking.food_id, formContent.food_options)}</Typography>
              ...
              <Typography variant="body1"><strong>Amount Shifts:</strong> {selectedBooking.amount_shifts}</Typography>
              <Typography variant="body1"><strong>Supporter Buddy:</strong> {selectedBooking.supporter_buddy}</Typography>
