...
function WorkShift({workShift, currentBooking, updateBooking, availablePriorities}: WorkShiftProps) {
	...
	return (
		<Box sx={{display: 'flex', 'flexDirection': 'column', 'alignItems': 'center', 'justifyContent': 'center', maxWidth: '90vw', marginTop: '8px'}}>
			<Typography align={"center"} variant="h5">{workShift.title}</Typography>
			<Typography align={"center"} variant="body2">{workShift.description}</Typography>
