...
const uiStateReducer = (state: AssignmentUIState, action: AssignmentAction): AssignmentUIState => {
    switch (action.type) {
        ...
        case 'TOGGLE_TIMESLOT_SELECTION':
            ...
            const newSelectedTimeslots = new Set<number>();
            if (!state.selectedTimeslots.has(action.payload)) {
        ...
        case 'SET_TIMESLOT_DETAILS':
            return {
                ...state,
                selectedTimeslotForDetails: action.payload,
                // Also update the selection when setting details
                selectedTimeslots: action.payload ? new Set([action.payload]) : new Set()
            };
    ...
    const getSortedUsers = useCallback((): BookingSummary[] => {
        ...
        if (uiState.selectedTimeslotForDetails) {
            sorted = sorted.sort((a, b) => {
                // Safely check if priority_timeslots exists and has values
                const aPriorityTimeslots = a.priority_timeslots || {};
                const bPriorityTimeslots = b.priority_timeslots || {};

                const aHasPriority = Object.values(aPriorityTimeslots).includes(uiState.selectedTimeslotForDetails!);
                const bHasPriority = Object.values(bPriorityTimeslots).includes(uiState.selectedTimeslotForDetails!);

                if (aHasPriority && !bHasPriority) return -1;
                if (!aHasPriority && bHasPriority) return 1;

                // Then by priority level (lower number = higher priority)
                const aPriority = Object.entries(aPriorityTimeslots)
                    .find(([_, timeslotId]) => timeslotId === uiState.selectedTimeslotForDetails)?.[0];
                const bPriority = Object.entries(bPriorityTimeslots)
                    .find(([_, timeslotId]) => timeslotId === uiState.selectedTimeslotForDetails)?.[0];
            ...
            <Box sx={{
                ...
                <Box sx={{gridColumn: 1, overflow: 'scroll'}}>
                    <UserList
                        ...
                        onFilterChange={(showUnassigned) => dispatchUI({
                            type: 'SET_SHOW_ONLY_UNASSIGNED',
                            payload: showUnassigned
                        })}
                        onSortChange={(sortBy, sortOrder) => dispatchUI({
                            type: 'SET_SORT',
                ...
                <Box sx={{gridColumn: 3, overflow: 'hidden'}}>
                    <TimeslotList
                        ...
                        onTimeslotSelect={(timeslotId) => dispatchUI({
                            type: 'TOGGLE_TIMESLOT_SELECTION',
                            payload: timeslotId
                        })}
                        onTimeslotDetails={(timeslotId) => dispatchUI({
                            type: 'SET_TIMESLOT_DETAILS',
                            payload: timeslotId
                        })}
                        onFilterChange={(showAvailable) => dispatchUI({
                            type: 'SET_SHOW_ONLY_AVAILABLE',
                            payload: showAvailable
                        })}
